...
@torch.no_grad()
def translate_images(nets, args, x_src, x_ref, y_ref, filename):
    N, C, H, W = x_src.size()
    ...
    for i, s_ref in enumerate(s_ref_list):
        ...
        for j in range(N):
            save_image(x_fake_with_ref[j,:,:,:],1,ospj(filename,'src_{}-ref_{}.jpg'.format(j,i)))
...
def video_ref(nets, args, x_src, x_ref, y_ref, fname):
    ...
    for data_next in tqdm(zip(x_ref, y_ref, s_ref), 'video_ref', len(x_ref)):
        ...
        frames = torch.cat([interpolated], dim=3).cpu()  # (T, C, 256*2, 256*(batch+1))
        video.append(frames)
        x_prev, y_prev, s_prev = x_next, y_next, s_next
    ...
    count = 0
    for f in video[0]:
        vutils.save_image(f,'./expr/results/afhq/{}.jpg'.format(count))
        count += 1
